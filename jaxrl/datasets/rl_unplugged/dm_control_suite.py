  ...
  """
    ...
    def step(self, action):
        ...
        if self._rescale == 'tanh':
            ...
            scaled_actions = tree.map_structure(lambda a: np.clip(a, -1., 1.),
                                                action)
        else:
...
class NormilizeActionSpecWrapper(wrappers.EnvironmentWrapper):
    ...
    def __init__(self, environment):
        ...
        self._action_spec = specs.BoundedArray(action_spec.shape,
                                               action_spec.dtype,
                                               minimum,
                                               maximum,
...
class CmuThirdParty:
    ...
    def __init__(self, task_name='humanoid_walls'):
        ...
        self._proprio_keys = [
            'walker/joints_vel', 'walker/sensors_velocimeter',
            'walker/sensors_gyro', 'walker/joints_pos', 'walker/world_zaxis',
            'walker/body_height', 'walker/sensors_accelerometer',
    ...
    for k in parsed.keys():
        ...
        if k in uint8_features:
            observation[k.replace('observation/', '')] = tf.reshape(
                tf.io.decode_raw(parsed[k], out_type=tf.uint8),
                (-1, ) + shapes[k])
        else:
...
def _padded_batch(example_ds, batch_size, shapes, drop_remainder=False):
    ...
    for k, v in shapes.items():
        if 'observation' in k:
            padded_shapes['observation'][k.replace('observation/',
                                                   '')] = (-1, ) + v
        else:
            padded_shapes[k] = (-1, ) + v
...
def dataset(root_path: str,
    ...
    filenames = [
        f'{path}-{i:05d}-of-{num_shards:05d}' for i in range(num_shards)
    ]
    file_ds = tf.data.Dataset.from_tensor_slices(filenames)

    example_ds = file_ds.interleave(functools.partial(tf.data.TFRecordDataset,
